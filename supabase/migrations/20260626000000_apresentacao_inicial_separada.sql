-- ============================================================================
-- "1ª Apresentação" separada do Agent Start. 3 blocos RÍGIDOS sem LLM:
--   Bloco 1 — texto institucional (editável)
--   Bloco 2 — foto + (header + lista auto produtos + footer opcional)
--   Bloco 3 — bônus (editável)
-- Reapresentação_meses também vive aqui.
--
-- Agent Start cuida APENAS do bloco 4 (saudação por canal OU resposta direta).
-- ============================================================================
...
INSERT INTO public.agent_config (agent, chave, valor, descricao) VALUES
  ...
  ('apresentacao', 'bloco2_foto_url',
   '"https://epreaawpvxrpqqthcczu.supabase.co/storage/v1/object/public/Start/TabelaOficial.png"'::jsonb,
   'Bloco 2 — URL da foto enviada como mídia (bucket Start).'),

  ('apresentacao', 'bloco2_header',
   '"📋 *Nosso cardápio:*"'::jsonb,
   'Bloco 2 — header acima da lista de produtos (na caption da foto).'),

  ('apresentacao', 'bloco2_footer',
   '""'::jsonb,
   'Bloco 2 — footer opcional abaixo da lista (cupom, frase de impacto). Vazio = não envia.'),
...
-- Migra reapresentar_meses de 'start' pra 'apresentacao' (se existir)
DO $$
DECLARE v_old jsonb;
BEGIN
