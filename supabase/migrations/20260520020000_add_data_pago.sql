-- Coluna data_pago: registra QUANDO o pedido foi pago.
-- Dashboard/Metricas usam esta coluna para o periodo de faturamento,
-- ticket medio, produtos vendidos, etc. (em vez de pedidos.data que
-- continua sendo a data de criacao do pedido).

ALTER TABLE public.pedidos ADD COLUMN IF NOT EXISTS data_pago date;
CREATE INDEX IF NOT EXISTS idx_pedidos_data_pago ON public.pedidos(data_pago) WHERE data_pago IS NOT NULL;

-- Backfill: pedidos ja pagos recebem data_pago = data (creation date)
UPDATE public.pedidos
SET data_pago = data
WHERE status_pagamento = 'pago' AND data_pago IS NULL;

-- criar_pedido_v2: preenche data_pago quando criar ja pago.
-- Pendentes ficam com data_pago = NULL ate serem marcados como pagos.
...
BEGIN
  ...
  IF p_produtos IS NOT NULL AND jsonb_typeof(p_produtos) = 'array' AND jsonb_array_length(p_produtos) > 0 THEN
    FOR v_produto_id, v_qtd IN SELECT (item->>'produto_id')::uuid, COALESCE((item->>'quantidade')::integer, 1) FROM jsonb_array_elements(p_produtos) AS item LOOP
      SELECT * INTO v_lote_rec FROM public.lotes l WHERE l.produto_id = v_produto_id AND l.uf = v_uf_postagem_calc ORDER BY l.data_producao ASC LIMIT 1;
      IF NOT FOUND THEN SELECT * INTO v_lote_rec FROM public.lotes l WHERE l.produto_id = v_produto_id ORDER BY l.data_producao ASC LIMIT 1; END IF;
      IF FOUND THEN
        UPDATE public.lotes SET quantidade_atual = quantidade_atual - v_qtd WHERE id = v_lote_rec.id;
        INSERT INTO public.estoque_movimentacoes (pedido_id, produto_id, quantidade, tipo, lote_id, uf_origem, observacao) VALUES (v_pedido_id, v_produto_id, v_qtd, 'saida', v_lote_rec.id, v_lote_rec.uf, 'Pedido #' || v_order_number::text);
      ELSE
        INSERT INTO public.estoque_movimentacoes (pedido_id, produto_id, quantidade, tipo, uf_origem, observacao) VALUES (v_pedido_id, v_produto_id, v_qtd, 'saida', v_uf_postagem_calc, 'Pedido #' || v_order_number::text || ' (sem lote)');
      END IF;
