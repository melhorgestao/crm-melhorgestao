-- ============================================================================
-- FIX: estoque_movimentacoes DUPLICADAS por venda.
--
-- Causa (confirmada em produção): DOIS escritores gravam a saída no INSERT do
-- pedido —
--   A) trigger de estoque (trg_novo_pedido_estoque) → "Pedido #<uuid>"
--   B) o RPC criar_pedido_v2 (inline)               → "Pedido #<order_number>"
-- A 20260520030000 já tinha removido a escrita do RPC exatamente por isso; a
-- 20260629 (fix de caixas) reintroduziu, e a 20260904 (itens free) carregou.
--
-- DECISÃO: o TRIGGER é a fonte única — cobre TODOS os caminhos de criação de
-- pedido (Financeiro E o webhook do DeFlow, que insere direto em pedidos). O
-- RPC só cobre o Financeiro. Então o RPC NÃO escreve estoque/lote; só grava
-- pedido + pedido_itens (com is_free) + lançamento.
--
-- (O SALDO de estoque é dinâmico, calculado de `pedidos` — nada aqui altera
--  saldo; só para de duplicar o LOG de movimentações.)
--
-- 1) Recria criar_pedido_v2 sem escrita de estoque/lote (mantém is_free).
-- 2) Limpa as duplicatas do RPC já existentes (as "Pedido #<order_number>"),
--    só quando existe a linha canônica do trigger ("Pedido #<uuid>") pro mesmo
--    pedido+produto. Seguro e reversível.
-- ============================================================================
...
BEGIN
  ...
  -- pedido_itens (rastreio + is_free). ESTOQUE/LOTE é do TRIGGER (fonte única).
  IF p_produtos IS NOT NULL AND jsonb_typeof(p_produtos) = 'array' AND jsonb_array_length(p_produtos) > 0 THEN
...
-- ── LIMPEZA das duplicatas já criadas pelo RPC ──────────────────────────────
-- Apaga a linha do RPC ("Pedido #<order_number>") só quando existe a linha
-- canônica do trigger ("Pedido #<uuid>") pro MESMO pedido+produto. Nunca apaga
-- se não houver a linha do trigger (evita zerar o histórico de algum pedido).
DO $$
DECLARE v_n int;
BEGIN
  WITH del AS (
    DELETE FROM public.estoque_movimentacoes rpc
     WHERE rpc.tipo = 'saida'
       AND rpc.observacao ~ '^Pedido #[0-9]+( \(sem lote\))?$'   -- formato do RPC
       AND EXISTS (
         SELECT 1 FROM public.estoque_movimentacoes trg
          WHERE trg.pedido_id = rpc.pedido_id
            AND trg.produto_id = rpc.produto_id
            AND trg.tipo = 'saida'
            AND trg.id <> rpc.id
            AND trg.observacao ~ '^Pedido #[0-9a-f]{8}-'         -- formato do trigger (uuid)
       )
    RETURNING 1
  )
  SELECT count(*) INTO v_n FROM del;
  RAISE NOTICE 'Duplicatas do RPC removidas: %', v_n;
END $$;
