...
export default function PedidosPage() {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 0 }) => {
      ...
      const { data } = await supabase.from('pedidos')
        .select('id, data, canal, order_number, valor, status_pedido, status_pagamento, produto, quantidade, uf_postagem, codigo_rastreio, contato_id, entrega_em_maos, estoque_debitado, locked_at, contatos(nome, telefone, tag_vip, cpf, endereco, complemento, bairro, cidade_uf, cep)')
  ...
  const {
    ...
    queryFn: async ({ pageParam = 0 }) => {
      const { data } = await supabase.from('pedidos')
        .select('id, data, canal, order_number, valor, status_pedido, status_pagamento, modalidade, uf_postagem, quantidade, contato_id, observacao, criado_por, codigo_rastreio, produto, locked_at, contatos(nome, telefone, cpf, endereco, complemento, bairro, cidade_uf, cep)')
