...
// Valor "Total" do pedido para a aba Lista:
// valor_original (inicial) - desconto_total acumulado.
// Fallback: se valor_original nao existe (pedidos legados sem backfill), usa valor.
const valorTotalLista = (p: any): number => {
  const original = p?.valor_original;
  if (original == null) return Number(p?.valor) || 0;
...
export default function PedidosPage() {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 0 }) => {
      const { data } = await supabase.from('pedidos')
        .select('id, data, canal, order_number, valor, valor_original, desconto_total, status_pedido, status_pagamento, modalidade, uf_postagem, quantidade, contato_id, observacao, criado_por, codigo_rastreio, produto, locked_at, is_free, contatos(nome, telefone, cpf, endereco, complemento, bairro, cidade_uf, cep, canal_origem)')
  ...
  const fetchRanking = async () => {
    ...
    data.forEach((p: any) => {
      ...
      byContact[id].totalValor += valorTotalLista(p);
  ...
  const exportCSV = () => {
    ...
    filteredPedidos.forEach(p => {
      rows.push([`#${p.order_number}`, formatDateShort(p.data), (p.contatos as any)?.nome || '', p.canal, String(valorTotalLista(p)), p.status_pedido === 'postado' ? 'Postado' : p.status_pedido === 'entregue' ? 'Entregue' : 'Aguardando Postagem', (p.contatos as any)?.telefone || '']);
    });
  ...
  const totalSum = filteredPedidos.reduce((s, p) => s + (p.is_free ? 0 : valorTotalLista(p)), 0);
            ...
            {filteredPedidos.map(p => (
              <div key={p.id} className="p-3 border rounded-xl hover:bg-muted/30">
                ...
                <div className="flex justify-between items-center gap-2">
                  <span className="text-sm font-bold cursor-pointer" onClick={() => setDetailPedido(p)}>{p.is_free ? <span className="text-sky-600">FREE</span> : formatBRL(valorTotalLista(p))}</span>
            ...
            {contactPedidos.map(p => {
              ...
              return (
                <div key={p.id} className="border-b border-border/50 py-2">
                  <p>{prodDisplay} — {p.is_free ? <span className="text-sky-600 font-bold">FREE</span> : formatBRL(valorTotalLista(p))} — {formatDateShort(p.data)} — {p.status_pedido === 'entregue' ? '✅ Entregue' : p.status_pedido === 'postado' ? 'Postado' : 'Aguardando'}</p>
                </div>
              );
            })}
        ...
        <DialogContent>
          ...
          {detailPedido && (
            <div className="space-y-2 text-sm">
              ...
              <p><strong>Valor Total:</strong> {detailPedido.is_free ? <span className="text-sky-600 font-bold">FREE</span> : formatBRL(valorTotalLista(detailPedido))}</p>
