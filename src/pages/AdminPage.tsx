...
import { ChevronLeft, ChevronRight, Plus, Trash2, Shield, Loader2, Users, ScrollText, Settings2 } from 'lucide-react';
...
export default function AdminPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-display font-bold tracking-tight">Administração</h1>

      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-muted/60 rounded-full p-1 h-auto">
          <TabsTrigger value="usuarios" className="rounded-full px-4 py-1.5 gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm"><Users className="w-3.5 h-3.5" /> Usuários</TabsTrigger>
          <TabsTrigger value="logs" className="rounded-full px-4 py-1.5 gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm"><ScrollText className="w-3.5 h-3.5" /> Logs</TabsTrigger>
          <TabsTrigger value="sistema" className="rounded-full px-4 py-1.5 gap-1.5 data-[state=active]:bg-background data-[state=active]:shadow-sm"><Settings2 className="w-3.5 h-3.5" /> Sistema</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="usuarios" className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={() => { resetCreateForm(); setShowCreateUser(true); }} className="rounded-full bg-emerald-600 hover:bg-emerald-700 text-white">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border/60 bg-muted/30 text-[11px] uppercase tracking-wider text-muted-foreground">
                    <th className="text-left font-semibold px-4 py-2.5 w-[18%]">Apelido</th>
                    <th className="text-left font-semibold px-4 py-2.5 w-[22%]">Email</th>
                    <th className="text-left font-semibold px-4 py-2.5 w-[16%]">Tipo</th>
                    <th className="text-left font-semibold px-4 py-2.5 w-[12%]">Serviço/UF</th>
                    <th className="text-left font-semibold px-4 py-2.5 w-[12%]">Instância</th>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border/60 bg-muted/30 text-[11px] uppercase tracking-wider text-muted-foreground">
                    <th className="text-left font-semibold px-4 py-2.5 w-[25%]">Apelido</th>
                    <th className="text-left font-semibold px-4 py-2.5 w-[20%]">Canal</th>
                    <th className="text-left font-semibold px-4 py-2.5 w-[20%]">Criado em</th>
                    <th className="text-left font-semibold px-4 py-2.5 w-[15%]">É Sócio?</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  {adminContatos.map(c => {
                    ...
                    return (
                    <tr key={c.id} className="border-b border-border/40 last:border-0 hover:bg-muted/40 transition-colors">
                      <td className="px-4 py-2.5 font-medium">{c.nome}</td>
                      <td className="px-4 py-2.5 text-muted-foreground">{c.canal_origem}</td>
                      <td className="px-4 py-2.5 text-muted-foreground">{formatDateTime(c.created_at)}</td>
                      <td className="px-4 py-2.5">
                        {isSocio ? (
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700">Sim</span>
                        ) : (
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-muted text-muted-foreground">Não</span>
                        )}
                      </td>
                    </tr>
                    );
            ...
            <>
              <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border/60 bg-muted/30 text-[11px] uppercase tracking-wider text-muted-foreground">
                      <th className="text-left font-semibold px-4 py-2.5">Data/Hora</th>
                      <th className="text-left font-semibold px-4 py-2.5">Usuário</th>
                      <th className="text-left font-semibold px-4 py-2.5">Ação</th>
                      <th className="text-left font-semibold px-4 py-2.5">Detalhe</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pagedLogs.length === 0 && (
                    ...
                    {pagedLogs.map(l => (
                      <tr key={l.id} className="border-b border-border/40 last:border-0 hover:bg-muted/40 transition-colors">
                        <td className="px-4 py-2.5 text-muted-foreground">{formatDateTime(l.created_at)}</td>
                        <td className="px-4 py-2.5 font-medium">{l.usuario}</td>
                        <td className="px-4 py-2.5">{l.acao}</td>
                        <td className="px-4 py-2.5 text-muted-foreground">{l.detalhe || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <Dialog open={showCreateUser} onOpenChange={(open) => { setShowCreateUser(open); if (!open) resetCreateForm(); }}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle className="font-display tracking-tight">Novo Usuário</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
            ...
            {formTipo === 'admin' && (
              <div className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <Switch checked={formIsSocio} onCheckedChange={setFormIsSocio} className="data-[state=checked]:bg-emerald-600" />
              </div>
            )}
            ...
            <Button onClick={handleCreateUser} disabled={creatingUser} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white min-h-[44px]">
